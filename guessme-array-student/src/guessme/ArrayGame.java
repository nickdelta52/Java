...
public class ArrayGame {
	...
	public ArrayGame() {
		guessed = new boolean[10000];
		eliminated = new boolean[10000];
	}
	...
	public boolean updateGuess(int nmatches) {
	 ...
	 for(int i = 1000; i<eliminated.length; i++)
	  ...
	  for(int j = 1000; j<eliminated.length; j++)
	  {if(eliminated[j] == false)
	  {guess = j;
	  return true;}
	  }
	...
	public int[] priorGuesses() {
	    ...
	    int count2 = 0;
		for(int i = 1000; i < guessed.length; i++)
		{
			  if(guessed[i])
				{guesses[count2] = i;
				count2++;}
